...
test("@critical account export download has exact current-account contract and no owner fields", async ({ page }) => {
  ...
  expect(payload.exportVersion).toBe("skillmint-account-export-v5");
  expect(payload.schemaContractVersion).toBe("skillmint-account-contract-v4");
  ...
  expect(payload.data.active_resume_selections).toEqual([]);
  expect(
