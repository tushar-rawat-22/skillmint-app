...
const FINAL_VERSION = "20260912001400";
...
async function runTimedCleanMigration() {
  ...
  assert.equal(history.at(-1), FINAL_VERSION, "clean migration rehearsal did not reach V14");
  process.stdout.write(`CLEAN_V2_TO_V14_MIGRATION_MS=${elapsedMs}\n`);
}
...
async function runLockContentionMigration() {
  ...
  const history = await migrationHistory();
  assert.equal(history.at(-1), FINAL_VERSION, "migration did not recover to V14 after lock release");
