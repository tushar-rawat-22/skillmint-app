...
Block 6.1 and Block 6.2 are merged and frozen pending rollout. Block 6.2 passed independent review. The fail-closed application code was automatically deployed from `main`, but V5 and V6 remain unapplied. No Production database or setting was changed during the repository-foundation pass.

The isolated `skillmint-block6-test` project exists with status `ACTIVE_HEALTHY`. Use it for the separately authorized migration and live-security gate before considering Production.
...
| Isolated | No Vercel project connection | Supabase `skillmint-block6-test`; contains no Production data and is intended for V1–V6 bootstrap and live-security verification |
...
The ordered chain is recorded in `supabase/migrations/manifest.json`. Each timestamped migration must remain byte-identical to its named `supabase/schema_v1.sql` through `supabase/schema_v6_analytics_aggregation.sql` source.

Do not edit an applied migration in place. Investigate a mismatch and prepare a reviewed forward migration when a correction is necessary.
...
When a supported container runtime is available, run a local Supabase stack and database reset before linking a hosted project. The reset must apply the exact committed V1–V6 chain. Verify migration history, schema objects, and security contracts; command exit status alone is insufficient.

The current outer transaction wrappers remain unchanged. Do not transform V4–V6 unless an actual execution with the pinned CLI proves a specific incompatibility.
...
3. Run a database push dry-run and confirm the exact V1, V2, V3, V4, V5, V6 order.
4. Apply V1, V2, V3, V4, V5, and V6 in that order.
5. Run the catalog, privilege, RLS, raw-row denial, RPC, retention, and application security checks.
6. Remove the disposable workdir after evidence is captured safely.
...
After the baseline is proven, the Production database push dry-run must show only V5 and V6. Any other result blocks rollout and requires investigation or a reviewed forward migration.

Apply V5 and V6 separately during an authorized Production rollout. Capture and review evidence after V5 before proceeding to V6.

## 7. V5 verification

Verify the `analytics_events` table, exact columns and constraints, three expected indexes, forced RLS, and grants. Confirm `public`, `anon`, and `authenticated` have no table access and `service_role` has insert only.
...
## 9. Founder authorization setup

Configure `ANALYTICS_FOUNDER_USER_ID` only after V5/V6 security verification and WAF protection. Use the founder's authentic Supabase Auth UUID as server-runtime authorization configuration. Never place it in analytics rows, responses, client variables, docs, or logs.
...
## 10. Vercel environment separation
...
## 15. Rollback and forward-fix policy

Application rollback and database recovery are separate decisions. Redeploying older application code does not reverse V5 or V6.
...
## 16. Evidence and closure checklist
...
- [ ] Isolated V1–V6 order, dry-run, application, catalog, RLS, ACL, raw-row denial, RPC, and purge evidence reviewed.
- [ ] Production V1–V4 history and normalized catalog match exactly before any history repair.
- [ ] Production dry-run shows only V5 and V6.
